...
**Sana:** 2026-09-17. **Holat:** BEKOR — 2026-09-17-klassik-taxta-medallar-design.md bilan almashtirildi. Spec matnidan ongli chetga chiqishlar va mijoz tasdig'ini kutayotgan qarorlar — §13 da («deploy qil» dan oldin ko'rib chiqilsin).
...
- **Sana qatori:** hafta kuni Asia/Tashkent da hisoblanadi, lekin nomlar JADVALDAN (`dateLine.ts`) — Chromium ICU sida o'zbekcha oy/hafta kuni nomlari yo'q, `Intl('uz')` «17-M09 2026, Thu» chiqargan edi. `today` presetida «17-sentabr 2026, payshanba · bugun»; qolgan har oynada mock so'zlari bilan — «1–17 sentabr 2026», bir kun «16-sentabr 2026, chorshanba», oylar/yillar orasida «28 avgust – 3 sentabr 2026» (PageShell ning «1-sen 2026 – 17-sen 2026» oralig'i o'rniga; `meta` davrsiz uzatiladi).
...
## §13 Spec matnidan chetga chiqishlar — mijoz tasdig'i kerak (2026-09-17, ikki review'dan keyin)

Har biri ongli, testda pinlangan va qaytarish bir-ikki qatorlik. Spec matni mijoz tasdiqlagan matn bo'lgani uchun TV da boshqacha o'qiladigan joylar shu yerda yoziladi — «deploy qil» dan oldin foydalanuvchi ko'rib chiqsin.

1. **`queued` — IKKALA faktda pul yo'q** (§8 matni: «faol raqami 0»). FAKT 1 da puli bor, FAKT 2 o'qilayotgan sotuvchi navbat guruhiga emas, rank qatoriga tushadi (hero uyasi xira chiziqcha) — pul bor odam «tasdiq kutilmoqda» deb aytilmasin. `board.ts` `splitBoard`.
2. **«Bugun» komandalar — boshqa faktda puli bor komanda ham rank oladi** (§8 matni: «faqat faol raqami > 0 komandalar rank bilan»). Faqat ikkala faktda puli yo'q komandalar «Hali savdosiz» satriga o'tadi. `TeamsBoard.tsx`.
3. **FAKT 2 o'qilganda sarlavha sanog'i «bugun N sotuvchi/komanda yetkazdi»** (§8 faqat «savdo qildi» ni ta'riflaydi). FAKT 2 kaliti ostida «bugun 0 sotuvchi savdo qildi» 15 kishi FAKT 1 da savdo qilgan kunni yolg'on aytardi (real-data audit).
4. **«Avgustdan beri» so'zi 1280–1599 px da tushadi** (§4 da bu satr to'liq). O'rindiq ichki kengligi ~164 px — so'z bilan satr ikki qatorga o'ralib ro'yxatdan qator olardi; raqamlar qoladi, asosni sahifa tagi aytadi. Telefonda (≤ 1279) va 1600+ da so'z QOLADI (avval telefonda ham tushardi — review tuzatdi).
5. **Uzunroq davrda (Shu oy, Kecha…) «hech narsasi yo'q» sotuvchilar ro'yxat OXIRIDA, yangi «Savdosiz» guruh sarlavhasi ostida** («bu davrda tasdiqlangan ham, yetkazilgan ham pul yoʻq»). §8 ning «ro'yxatga chiqmaydi» qoidasi faqat «Bugun» ga tegishli deb o'qildi (bo'lim sarlavhasi «Bugun ertalab»). Avval bu qatorlar rank-siz, to'rt chiziqcha bilan navbatdagilarning USTIDA turardi. Muqobil: har oynada tushirish — mijoz aytsa bir qator. Production «Shu oy» (2026-09-17): 6 ta shunday sotuvchi, 1 ta navbatda.
6. **`--efir-ink-3` kontrasti har sirtda 4.5 emas.** Tokenlar `final-tokens.css` dan AYNAN (§1); test `--efir-panel` ga nisbatan o'lchaydi va o'tadi. Lekin qorong'i ink-3 #737e92 `--efir-raised-hi` da 3.85:1, `--efir-raised` da 4.31:1 (o'rindiq/sahna tepasidagi komanda nomi, kod tokeni, plaket sanog'i); yorug' ink-3 #677285 `--efir-sunken` da 4.25:1 (legenda ostonalari, jami yorliqlari). Tuzatish mock egasining qarori: qorong'i ink-3 ≈ #8691a5, yorug' ≈ #5f6a7c, yoki shu yorliqlarni ink-2 ga. **Tuzatildi (2026-09-17):** qorong‘i ink-3 #8691a5, yorug‘ #5f6a7c — endi har sirtda ≥ 4.5:1 (eng pasti qorong‘i `--efir-raised-hi` 4.96, yorug‘ `--efir-sunken` 4.79).
7. **`AnimatedNumber` bitta matn tuguni (§9) — umumiy primitiv, 8 o'quvchisi bor**: Stat, DataTable, faktVocabulary (ConfirmationFaktSection, ConfirmationOutcomeSection, ForecastSection, LogisticsPage), KPI, Margin, Cohort, Warehouse. Ko'rinish o'zgarmaydi, lekin DOM va a11y o'zgaradi: ekran o'quvchi 900 ms gacha sanalayotgan oraliq qiymatni o'qishi mumkin (avval yashirin yakuniy nusxa bor edi). Bu `/confirmation` ga ham yetadi (xotira: u ekran «tegilmasin»). `aria-busy` qo'shilmadi — umumiy faylga yana bir o'zgarish.
8. **Tokenlar `main:has(.tv-board-shell)` da ham e'lon qilinadi, sahifa nuri `main` da, PageShell «aurora» si bu sahifada chizilmaydi.** Rekord devori va davr boshqaruvi PageShell sarlavhasida — taxtadan tashqarida — va tokenlar faqat `.tv-board-shell` da bo'lganda plaketlar global palitrada tekis ramkali quti bo'lib chizilardi (§7 material yo'q edi). Aurora `--accent` (pushti) dan aralashtirilib sarlavha ustida magenta dog' qoldirardi; mockda faqat sovuq `--efir-page-glow` bor. Hammasi sahifaga xos selektor orqali — `Shell`/`PageShell` tahrirlanmadi.
9. **Ustun sarlavhasidagi izoh («FAKT 1 — tasdiqlangan pul») belgi soniga emas, JOYGA qarab tushadi.** Sanoq har doim to'liq; izoh qolgan joyga sig'sa chiziladi, sig'masa butunlay tushadi (hech qachon ellipsis). Natija: 1920 «Bugun» da komandalar izohi mockdagidek ko'rinadi; 1366 da komandalar ustunida va uzun «bugun … tasdiq kutmoqda» sanog'i bilan sotuvchilar ustunida tushadi.
10. **1280–1599 da legenda 3 × 2 jadval** (§7: 36 px bir qatorli tasma, `space-between`). Olti pog'ona ~835 px so'raydi, bu kadrda ustun ≤ ~850 — bir qatorga hech qachon sig'maydi; `space-between` o'ralishi ikki pog'onani ikki chetga surib ~300 px bo'shliq qoldirardi.
