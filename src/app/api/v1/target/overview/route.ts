 ...
 * registrar who picked it up, not to a seller's team, and the ad half of the
 * payload is the Roistat ledger, which has no employee on any row. There is no
 ...
 * of the seven target pages. The `ads` block is the ad ledger over the same
 * calendar days, clamped by that ledger to the days it holds.
 */
export const GET = getHandler(ACCESS, targetOverviewQuerySchema, async (ctx) => {
  const period = periodFrom(ctx.query, ctx.timeZone, ctx.now)
  const data = await targetService.overview(period, ctx.query.scope, ctx.timeZone, ctx.now)
