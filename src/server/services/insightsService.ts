...
export class InsightsService {
  ...
  /**
   * «Qoʻngʻiroqlar» — who spoke to customers, how long and when, on the
   * dashboard's own window.
   *
   * ONE STATEMENT, ONE KEY. The key is `period.start` plus the span in days —
   * the DAY it lands on rather than the instant — matching `customerFlow`'s key
   * and for the same reason: re-querying per request would put a statement
   * measured in seconds from Tashkent into a one-vCPU database once per reader
   * per minute.
   *
   ...
   *
   *   - this screen states NO MONEY. The day a soʻm figure appears here, the key
   *     needs a currency.
   ...
   * mattered there because it chose a COMPARISON window; nothing on this screen
  ...
  async callActivity(period: Period): Promise<CallActivityDto> {
    ...
    const activity = await callActivityCache.get(key, () =>
    ...
    const row = (source: CallActivityRow): CallRowDto => ({
      ...
      firstCallAt: source.firstCallAt,
    ...
    return {
      ...
      days: activity.days.map(row),
      hours: activity.hours.map(row),
      /*
