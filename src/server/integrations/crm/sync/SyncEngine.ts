 ...
 *
 * THREE HOURS, AND THE CONDITION IS THE LONGEST CALL, NOT THE PASS INTERVAL. A
 * call caught unfinished by pass N started less than one call-length before
 * that pass; pass N+1 re-reads from its own start minus the lookback, which
 * reaches back past that call whenever the lookback exceeds the longest call.
 * The longest measured is 3 600 s, so three hours carries a threefold margin —
 * whatever the schedule. (It was written when CALLS ran half-hourly; the
 * reference clock went to three-hourly the same day, `REFERENCE_EVERY` 180, and
 * nothing here needed to change.) Roughly 60–120 invocations a pass against the
