 ...
 * bookmark lands on. That is about 65% of what this screen downloads, and all
 * of it had to parse before hydration, which is before the first `/api/v1`
 * request is even issued. The reader was waiting on a charting library to
 * compile in order to be shown a skeleton.
 *
 ...
 *
 * Without this the import is triggered by the render that first has data — so
 * the chunk download starts only once the API answers, and the two costs that
 * used to overlap are now serialised. Warming it on mount puts the download
 * inside the window the page spends waiting on `/dashboard/command`, which on
 * this database is comfortably longer than a 109 KB fetch.
 *
